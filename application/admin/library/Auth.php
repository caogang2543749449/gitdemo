...
class Auth extends \fast\Auth
{
    ...
    public function login($username, $password, $keeptime = 0)
    {
        ...
        //计算登录失败次数，失败5次时需等待10分钟，此后每多失败一次，等待时间延长1倍
        $failureWait = $admin->loginfailure - 5;
        if (Config::get('fastadmin.login_failure_retry') && $failureWait >= 0) {
            $leftMinutes = (1<<$failureWait)*10 - intval((time() - $admin->updatetime)/60);
            if($leftMinutes > 0) {
                $this->setError(__('Please try again after %s minutes', $leftMinutes));
                return false;
            }
        }
    ...
    public function logout()
    {
        ...
        Session::delete("org_merchant_id");
    ...
    public function getSidebar($params = [], $fixedPage = 'dashboard')
    {
        ...
        foreach ($ruleList as $k => &$v) {
            ...
            if( !empty($condition) && $condition==='merchant_selected' && !session('?merchant')) {
                unset($ruleList[$k]);
                continue;
            }
            $v['icon'] = $v['icon'] . ' fa-fw';
